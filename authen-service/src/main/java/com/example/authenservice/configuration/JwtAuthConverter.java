...
public class JwtAuthConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    private String getPrincipalClaimName(Jwt jwt) {
        String claimName = StringUtils.hasLength(principalAttribute) ? principalAttribute : JwtClaimNames.SUB;
    ...
    private Collection<? extends GrantedAuthority> extractResourceRoles(Jwt jwt) {
        ...
        if (resourceAccess != null && resourceAccess.get(resourceId) != null) {
            Map<String, Object> map = (Map<String, Object>) resourceAccess.get(resourceId);
            if (map.containsKey("roles")) {
                resourceRoles = (Collection<String>) map.get("roles");
                allRoles.addAll(resourceRoles);
            }
        }
